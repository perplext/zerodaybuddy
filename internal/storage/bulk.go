...
// BulkCreateHosts inserts multiple hosts in a single transaction with prepared statements.
func (s *SQLiteStore) BulkCreateHosts(ctx context.Context, hosts []*models.Host) error {
	if len(hosts) == 0 {
		return nil
	}
	...
	for _, host := range hosts {
		...
		if err != nil {
			return fmt.Errorf("failed to insert host %s: %w", host.Value, err)
		}
	}
...
// BulkCreateEndpoints inserts multiple endpoints in a single transaction with prepared statements.
func (s *SQLiteStore) BulkCreateEndpoints(ctx context.Context, endpoints []*models.Endpoint) error {
	if len(endpoints) == 0 {
		return nil
	}
	...
	for _, ep := range endpoints {
		...
		if err != nil {
			return fmt.Errorf("failed to insert endpoint %s: %w", ep.URL, err)
		}
	}
...
// BulkCreateFindings inserts multiple findings in a single transaction with prepared statements.
func (s *SQLiteStore) BulkCreateFindings(ctx context.Context, findings []*models.Finding) error {
	if len(findings) == 0 {
		return nil
	}
	...
	for _, finding := range findings {
		...
		if err != nil {
			return fmt.Errorf("failed to insert finding %s: %w", finding.Title, err)
		}
	}
